...
st.write("凡例名の変更欄を完全に独立させ、X軸が1つでも全てのグラフが正しく同時に表示されるよう修正しました。")
...
if not df.empty:
    ...
    if len(columns) < 2:
        ...
        if not x_axes or not y_axes:
            ...
            # 🛠️【描画ループの全面刷新】選択されたすべてのY軸を網羅して描画
            for y_idx, y_axis in enumerate(y_axes):
                # 対応するX軸をマッピング（X軸の数よりY軸が多くても、最後のX軸を安全に使い回す）
